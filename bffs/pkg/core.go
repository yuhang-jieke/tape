...
func (m Message) GenDefaultMessage(buf *bytes.Buffer) {
	...
	for _, field := range m.Fields {
		...
		if m.Style == fieldStyleToSnake {
			field.Name = From(field.Name).ToSnake()
		}
...
func (m Message) GenRpcGetByIdReqMessage(buf *bytes.Buffer) {
	...
	name := From(firstWord + mOrginName[1:]).ToCamelWithStartLower()
	comment := From(firstWord + mOrginName[1:]).ToCamelWithStartLower()
	if m.Style == fieldStyleToSnake {
		name = From(firstWord + mOrginName[1:]).ToSnake()
		comment = From(firstWord + mOrginName[1:]).ToSnake()
	}
	m.Fields = []MessageField{
